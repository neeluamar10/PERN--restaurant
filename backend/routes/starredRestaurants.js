...
router.post('/', (req, res) => {
  ...
  STARRED_RESTAURANTS.push(new_restaurant);
  res.status(200).send({
    id: new_restaurant.id,
    comment: new_restaurant.comment,
    name: restaurant.name
  });
...
router.delete('/:id', (req, res) => {
  const { id } = req.params;
  const filteredRestaurants = STARRED_RESTAURANTS.filter(restaurant => restaurant.id !== id);

  if(STARRED_RESTAURANTS.length === filteredRestaurants.length){
    res.statusCode(404);
    return;
  }
